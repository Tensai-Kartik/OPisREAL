...
export default function VictoryCardModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-950/85 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-lg parchment-panel rounded-2xl border-2 border-amber-500/80 shadow-2xl overflow-hidden p-4 sm:p-5 text-center relative max-h-[95vh]">
        {/* Header Badge */}
        {isSurrender ? (
          <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-red-500/20 border border-red-500/40 text-red-300 font-bold text-[11px] uppercase tracking-widest mb-2">
            <Flag className="w-3.5 h-3.5 text-red-400" />
            <span>Mystery Revealed</span>
          </div>
        ) : (
          <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-[11px] uppercase tracking-widest mb-2">
        ...
        <div className="grid grid-cols-2 gap-1.5 text-left text-[11px] bg-slate-950/80 p-3 rounded-xl border border-slate-800 mb-4">
          ...
          <div className="p-1.5">
            ...
            <span className="font-bold text-amber-200 truncate block">
              {character.origin} • {character.first_appearance || character.first_arc || 'Unknown'}
            </span>
          </div>
        </div>
        ...
        <button
          onClick={onPlayAgain}
          className="w-full py-3 gold-button rounded-xl text-xs sm:text-sm uppercase font-black tracking-wider flex items-center justify-center space-x-2 shadow-lg"
        >
